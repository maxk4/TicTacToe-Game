 ...
 * Allows user ot play a Tic Tac Toe game on boards with lengths of 3,4, or 5
 * for width/height.
 * A win is 3 in a row.
...
public class TicTacToe extends Canvas {
     ...
     *
     * @param board The 2D array board of size rows (dimension 1) and columns
     *              (dimension 2)
    ...
    /**
     * This function determines if the game is complete due to a win or tie by
     * either player
     *
     * @param board The 2D array board of size rows (dimension 1) and columns
     *              (dimension 2)
    ...
    private static void run() {
        ...
        // Get user choice of piece (X goes first so picking X means user goes first,
        // picking O means computer goes first
        ...
        while (!isGameOver(board)) {
            // Determine complexity guess about game options remaining (no rotational
            // considerations)
            ...
            if (human == player) {
                ...
                canvas.repaint();
                // Depending on flag get input for user playing via GUI or via input() prompts
                // in shell
                if (gui_flag) {
    ...
    private static boolean isInputValid(String value, int start, int end) {
        // Lazy way to check if the given string is in an integer range without
        // attempting to parse
        // This is a bad way to do this (only done to leave exception for students until
        // later)
    ...
    private static int inputDifficulty(int[][] board) {
        ...
        // We limit difficulty for boards that are not 3x3 (different message and upper
        // limit)
    ...
    private static void inputHint(int[][] board, int human, int computer, String h) {
        ...
        if (h.equals("h")) {
            ...
            // Tell player to block CPU if that exists second
                ...
                System.out.printf("Hint to stop opponent win in (%d,%d)%n", opponent_win_block[0],
                        opponent_win_block[1]);
                draw_hint = true;
    ...
    private static void inputMouseNextPlay(int[][] board, int human) {
        ...
        while (true) {
            ...
            // If math says the row,col calculation isn't in board then loop for another
            // click
     ...
     * We only need the mouse clicked event in this program but are required to
    ...
    private static class MyMouse extends JComponent implements MouseListener {
        ...
        public void mouseClicked(MouseEvent e) {
            // When a mouse is clicked we stored that value and notify anyone that might be
            // waiting for it
            mouse[0] = e.getX();
            mouse[1] = e.getY();
            synchronized (mouse) {
    ...
    /**
     * This allows us to re-paint the GUI window regularly (we have to update game
     * state variables to draw different things
     *
    ...
    /**
     * Set up the Java Swing window (really simple window a canvas and a mouse
     * listener)
     */
    ...
    private static void drawBoard(Graphics2D graphics) {
        ...
        for (int row = 0; row < board.length; row++) {
            for (int col = 0; col < board[0].length; col++) {
                if (board[row][col] == X) {
                    drawX(graphics, col * col_pixel_size, row * row_pixel_size, col_pixel_size, row_pixel_size,
                            draw_board_colour);
                } else if (board[row][col] == O) {
                    drawO(graphics, col * col_pixel_size, row * row_pixel_size, col_pixel_size, row_pixel_size,
                            draw_board_colour);
                }
            }
        }
        ...
        if (draw_game_over) {
            ...
            // Use font metrics to try and center the text over a white box for readability
            // (text and fonts are messy, sigh)
            ...
            graphics.fillRect(start_x - TEXT_BORDER, start_y_rect - TEXT_BORDER, stringLen + TEXT_BORDER * 2,
                    font_ascent + font_descent + TEXT_BORDER * 2);
    ...
    private static void drawX(Graphics2D graphics, int x, int y, int size_x, int size_y, Color colour) {
        ...
        graphics.setStroke(STROKE);
        graphics.drawLine(x + X_O_PIXELS_BORDER, y + X_O_PIXELS_BORDER, x + size_x - X_O_PIXELS_BORDER,
                y + size_y - X_O_PIXELS_BORDER);
        graphics.drawLine(x + size_x - X_O_PIXELS_BORDER, y + X_O_PIXELS_BORDER, x + X_O_PIXELS_BORDER,
                y + size_y - X_O_PIXELS_BORDER);
    ...
    private static void drawO(Graphics2D graphics, int x, int y, int size_x, int size_y, Color colour) {
        graphics.setColor(colour);
        graphics.setStroke(STROKE);
        graphics.drawOval(x + X_O_PIXELS_BORDER, y + X_O_PIXELS_BORDER, size_x - X_O_PIXELS_BORDER * 2,
                size_y - X_O_PIXELS_BORDER * 2);
    }

    /**
     * Draw hint information and X or O based on piece in given row, col of board
     * Quite similar to pieces from drawBoard/drawX/drawO as we are drawing a
     * sub-part of board in different colour for hint
     *
     * @param graphics Where we are drawing to
     */
    ...
    /**
     * Calling AI, if level 4 we do full recursive minimax, if not we recurse only
     * to certain depth
    ...
    /**
     * Minimax suggest of what row, col to play in for player1 as initial call, and
     * player as current tree call
     *
    ...
    private static Integer[] minimax(int[][] board, int player1, int player2, int player, int depth, int max_depth) {
        ...
        for (Integer[] move : moves) {
            ...
            // Depending on if we are currently on player1 or player 2 we update the best
    ...
    /**
     * Evaluate the board, 10 for player 1 win, -10 for player 1 loses, 0 for
     * neutral
     * We adjust this starting score about how far away this win is, so that the
     * result is a struggle
     *
